		...
		Public Property WizardModel() As CustomQueryWizardModel
			Get
				If Session("WizardModel") Is Nothing Then
					Return New CustomQueryWizardModel() With {
						.DataSources = New List(Of DataSourceInfo)() From {
							New DataSourceInfo() With {
								.Name = "Northwind", .ConnectionName = "NorthwindConnection", .Queries = New List(Of QueryInfo)() From {
									New QueryInfo() With {.Name = "Categories", .Sql = "SELECT * FROM [Categories]"},
									New QueryInfo() With {.Name = "Products", .Sql = "SELECT * FROM [Products]"}
								}
							},
							New DataSourceInfo() With {
								.Name = "Departments", .ConnectionName = "DepartmentsConnection", .Queries = New List(Of QueryInfo)() From {
									New QueryInfo() With {.Name = "Departments", .Sql = "SELECT * FROM [Departments]"}
								}
							}
						}
					}
